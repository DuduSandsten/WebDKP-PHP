...
DKP = new (function() {
	...
	this.Setup = function() {
		Event.observe(window,'load', DKP.SetupOnLoad);
	}
	...
	this.SetupNoItemFoundLink = function(link) {
		link.setAttribute("tooltip","Item data not available. Either Wowhead was busy or the item does not exist. Click to try loading data again.");
		link.setAttribute("icon","INV_Misc_QuestionMark");
		Event.observe(link, 'click', DKP.DownloadData);
	}
...
var DKPTable = Class.create({
	...
	GetPageButton: function(name) {
		...
		if(name == "first")
			...
			content = "Next >";
		var button = Builder.node('a',{href:'javascript:;',className:'pagebutton'});
	...
	GeneratePageBar: function() {
		var container = Builder.node('div',{style:'padding:2px',className:this.tableName+'_pagebar'});
	...
	DrawPageButtons: function() {
		if(!this.usePaging || this.pageLinksCreated)
			return;
...
var PointsTable = Class.create(ManualPageTable, {
    ...
    var armory = Builder.node('a',{href:"http://armory.twinstar.cz/character-sheet.xml?cn="+this.items[i].player+"&r=Kronos"},img);
...
//    row.appendChild(Builder.node('td',{},Builder.node('a',{href:DKP.BaseUrl+"Player/"+this.items[i].player,class:'player_link'},this.items[i].player)));
    ...
    row.appendChild(Builder.node('td',{},
                      ...
                      Builder.node('h6',{},
                        Builder.node('a',{href:"http://armory.twinstar.cz/character-sheet.xml?cn="+this.items[i].player+"&r=Kronos",class:'armory_link'},'Armory')
                      )
                    )
                  );
    ...
    row.appendChild(Builder.node('td',{},
      Builder.node('a',{href:"http://armory.twinstar.cz/guild-info.xml?gn="+this.items[i].playerguild+"&r=Kronos"},this.items[i].playerguild))
    );
...
var RemotePointsTable = Class.create(DKPTable, {
	...
	GetRow: function(i) {
		...
		row.appendChild(Builder.node('td',{},
				Builder.node('a',{href:Site.SiteRoot+WebDKP.BaseUrl+"Player/"+this.items[i].player,target:"WebDKP"},this.items[i].player)));

		var img = Builder.node('img',{src:Site.SiteRoot+"images/classes/small/"+this.items[i].playerclass+".gif"});

		row.appendChild(Builder.node('td',{className:"center",sortkey:this.items[i].playerclass},img));
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].dkp+""));
...
var PlayerLootTable = Class.create(DKPTable, {
	...
	GetRow: function(i) {
		...
		row.appendChild(Builder.node('td',{sortkey:this.items[i].date},this.items[i].datestring));

		var cell = Builder.node('td');
		cell.innerHTML = this.items[i].name;
		row.appendChild(cell);

		row.appendChild(Builder.node('td',{className:"center"},this.items[i].points+""));

		Event.observe(row,'mouseover', this.OnRowOver);
		Event.observe(row,'mouseout', this.OnRowOut);

		//var url = Site.SiteRoot + "dkp/"+DKP.ServerUrl+"/" + this.items[i].name;
		//Event.observe(row,'click', this.OnRowClick.bindAsEventListener(this, url));
...
var PlayerHistoryTable = Class.create(ManualPageTable, {
	...
	GetRow: function(i) {
		...
		if( this.items[i].foritem == 1 )
			...
			name.appendChild(Builder.node('a',{href:DKP.BaseUrl+"Award/"+this.items[i].id},this.items[i].name))
		...
		row.appendChild(Builder.node('td',{className:"center"},this.runningTotal));
		...
		if(this.canedit) {
			...
			//edit link
			var url = DKP.BaseUrl+"Admin/EditAward/" + this.items[i].id + "?b=p&pid=" + this.playerid+"&p="+this.page+"&s="+this.sortString+"&o="+this.orderString;
			var editImg = Builder.node('img',{src:Site.SiteRoot+"images/buttons/edit.png",style:"vertical-align:text-bottom"});
			var editLink = Builder.node('a',{href:url,className:"dkpbutton"},editImg);
			actions.appendChild(editLink);
			//delete link
		 	url = DKP.BaseUrl+"Player/"+this.playername+"/"+this.page+"/"+this.sortString+"/"+this.orderString+"?event=deleteHistory&historyid="+this.items[i].historyid;
			var deleteImg = Builder.node('img',{src:Site.SiteRoot+"images/buttons/delete.png",style:"vertical-align:text-bottom"});
			var deleteLink = Builder.node('a',{href:url,className:"dkpbutton",onclick:"return confirm('Delete Award?')"},deleteImg);
			//append
...
var AwardTable = Class.create(ManualPageTable, {
	...
	GetRow: function(i) {
		var row = Builder.node('tr');
		row.appendChild(Builder.node('td',{},
			Builder.node('a',{href:DKP.BaseUrl+"Award/"+this.items[i].id},this.items[i].name)));
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].points+""));
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].players+""));
		row.appendChild(Builder.node('td',{className:"center",sortkey:this.items[i].date},this.items[i].datestring));

		if(this.canedit) {
			var actions = Builder.node('td',{className:"center"});
			//edit link
			var url = DKP.BaseUrl+"Admin/EditAward/"+this.items[i].id+"?b=a&p="+this.page+"&s="+this.sortString+"&o="+this.orderString;
			var editImg = Builder.node('img',{src:Site.SiteRoot+"images/buttons/edit.png",style:"vertical-align:text-bottom"});
			var editLink = Builder.node('a',{href:url,className:"dkpbutton"},editImg);
			actions.appendChild(editLink);
			//delete link
		 	url = DKP.BaseUrl+"Awards/"+this.page+"/"+this.sortString+"/"+this.orderString+"?event=deleteAward&awardid="+this.items[i].id;
			var deleteImg = Builder.node('img',{src:Site.SiteRoot+"images/buttons/delete.png",style:"vertical-align:text-bottom"});
			var deleteLink = Builder.node('a',{href:url,className:"dkpbutton",onclick:"return confirm('Delete Award?')"},deleteImg);
			//append
			actions.appendChild(deleteLink);
...
var RemoteAwardTable = Class.create(DKPTable, {
	GetRow: function(i) {
		...
		row.appendChild(Builder.node('td',{},
			Builder.node('a',{href:Site.SiteRoot+WebDKP.BaseUrl+"Award/"+this.items[i].id},this.items[i].name)));
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].points+""));
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].players+""));
		row.appendChild(Builder.node('td',{className:"center",sortkey:this.items[i].date},this.items[i].datestring));
...
var LootTable = Class.create(ManualPageTable, {
	...
	GetRow: function(i) {
		...
		row.appendChild(cell);
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].points+""));
		row.appendChild(Builder.node('td',{className:"center"},this.items[i].player+""));
		row.appendChild(Builder.node('td',{className:"center",sortkey:this.items[i].date},this.items[i].datestring));

		if(this.canedit) {
			var actions = Builder.node('td',{className:"center"});
			//edit link
			var url = DKP.BaseUrl+"Admin/EditAward/"+this.items[i].id+"?b=l&p="+this.page+"&s="+this.sortString+"&o="+this.orderString;
			var editImg = Builder.node('img',{src:Site.SiteRoot+"images/buttons/edit.png",style:"vertical-align:text-bottom"});
			var editLink = Builder.node('a',{href:url,className:"dkpbutton"},editImg);
			actions.appendChild(editLink);
			//delete link
		 	url = DKP.BaseUrl+"Loot/"+this.page+"/"+this.sortString+"/"+this.orderString+"?event=deleteAward&awardid="+this.items[i].id;
			var deleteImg = Builder.node('img',{src:Site.SiteRoot+"images/buttons/delete.png",style:"vertical-align:text-bottom"});
			var deleteLink = Builder.node('a',{href:url,className:"dkpbutton",onclick:"return confirm('Delete Award?')"},deleteImg);
			//append
			actions.appendChild(deleteLink);
	...
	GetRow: function(i) {
		...
		for(var j = i ; j < i+5 ; j++ ) {
			...
			if( j < this.items.length ) {
				...
				var input = Builder.node('input',{name:"users[]",value:this.items[j].id,type:"checkbox",style:"vertical-align:bottom"},"");
				if(this.items[j].checked)
					input.checked = true;
				this.items[j].input = input;

				cell = Builder.node('td',{className:classname},[input," ",this.items[j].name]);
				this.items[j].cell = cell;

				Event.observe(cell,'click', this.OnItemClick.bindAsEventListener(this, j));
			}
			else {
				cell = Builder.node('td',{},"");
			}

			Event.observe(cell,'mouseover', DKPTable.OnRowOver);
			Event.observe(cell,'mouseout', DKPTable.OnRowOut);
